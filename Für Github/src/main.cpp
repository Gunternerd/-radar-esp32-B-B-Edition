...
static int gRadarDirection = -1;
static int gWindowMaxKmh = 0;
static int gWindowDirection = -1;
...
struct RadarConfig {
  ...
  int directionOverride; // -1=auto, 1=approach, 0=away
};

static RadarConfig gRadarConfig = {-45, 45, 1, SAMPLE_WINDOW_MS, 20, 2, 4, 12, 50, -1};
...
static int uploadToGoogleSheet(int speed, int limit, const char* source, int direction = -1);
...
static void normalizeRadarConfig() {
  ...
  if (!(gRadarConfig.directionOverride == -1 || gRadarConfig.directionOverride == 0 || gRadarConfig.directionOverride == 1)) {
    gRadarConfig.directionOverride = -1;
  }
}
...
static bool loadRadarConfigFromGoogle() {
  ...
  gRadarConfig.directionOverride = doc["direction_override"] | gRadarConfig.directionOverride;
...
static bool saveRadarConfigToGoogle() {
  ...
  String url = String(SECRET_GOOGLE_URL) + "?action=SET_RADAR_CONFIG"
               ...
               "&direction_override=" + String(gRadarConfig.directionOverride);
  http.begin(url);
  http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  const int httpCode = http.GET();
...
static void handleRadarConfigPage() {
  ...
  // Direction override select
  body += "Richtungsmodus<br><select name='direction_override'>";
  body += String("<option value='-1'") + (gRadarConfig.directionOverride==-1?" selected":"") + ">Auto</option>";
  body += String("<option value='1'") + (gRadarConfig.directionOverride==1?" selected":"") + ">Anfahrt</option>";
  body += String("<option value='0'") + (gRadarConfig.directionOverride==0?" selected":"") + ">Wegfahrt</option>";
  body += "</select><br>";
...
static void handleRadarConfigSave() {
  ...
  if (gWebServer.hasArg("direction_override")) {
    gRadarConfig.directionOverride = gWebServer.arg("direction_override").toInt();
  }
  normalizeRadarConfig();
...
static void handleDiagPage() {
  ...
  body += "<p>Limit: "; body += String(gRadarConfig.speedLimitKmh); body += " km/h</p>";
  body += "<p>Richtungsmodus: "; body += (gRadarConfig.directionOverride==-1?"Auto":(gRadarConfig.directionOverride==1?"Anfahrt":"Wegfahrt")); body += "</p>";
...
static void startWebServer() {
  ...
  gWebServer.on("/pdf_daily", HTTP_POST, []() {
    ...
    const int code = http.GET(); const String payload = code>0 ? http.getString() : String(); http.end();
    String body = navLinks(); body += "<div class='card'><h2>Tages-PDF</h2>";
    if (code==200 && payload.length()>0) { body += String("<p><a href='") + payload + String("' target='_blank'>PDF öffnen</a></p>"); }
  ...
  gWebServer.on("/google_test", HTTP_POST, []() {
    gLastHttpCode = uploadToGoogleSheet(77, 0, "WEBTEST", -1);
...
static const char* directionToText(int direction) {
  if (direction == 1) return "approach";
  if (direction == 0) return "away";
  return "unknown";
}

static int uploadToGoogleSheet(int speed, int limit, const char* source, int direction) {
#ifndef SECRET_GOOGLE_URL
    ...
    while (attempt < maxRetries) {
        ...
        String url = String(SECRET_GOOGLE_URL) +
                     ...
                     "&direction=" + String(directionToText(direction));
        http.begin(url);
        http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
        ...
        Serial.print(" direction="); Serial.print(directionToText(direction));
...
static void sendTelegramMessage(const String &text) {
  ...
  String payload = String("{\"chat_id\":\"") + String(ADMIN_ID) + "\",\"text\":\"";
  ...
  Serial.print("Telegram HTTP code: "); Serial.println(httpCode);
  http.end();
#endif
}
...
static void handleMeasurementWindow() {
  if (gSpeedKmh > gWindowMaxKmh) {
    ...
    gWindowDirection = gRadarDirection;
  }
  ...
  if (gWindowMaxKmh > 0) {
    ...
    int outDir = gWindowDirection;
    if (outDir < 0 && gRadarConfig.directionOverride != -1) {
      outDir = gRadarConfig.directionOverride;
    }
    const int httpCode = uploadToGoogleSheet(gWindowMaxKmh, gRadarConfig.speedLimitKmh, "RADAR", outDir);
    ...
    Serial.print(directionToText(gWindowDirection));
    Serial.print(" upload=");
    Serial.println(httpCode);

    if (uploadOk) {
      sendTelegramMessage(String("Radar: ") + gWindowMaxKmh + " km/h Richtung " + directionToText(gWindowDirection) + " um " + timestamp);
    }
  }
...
void setup() {
  ...
  delay(500);
  gLastHttpCode = uploadToGoogleSheet(77, 0, "TESTBOOT", -1);
