...
const DATA_HEADERS = ["Zeitstempel", "Geschwindigkeit", "Limit", "Überschreitung", "Status", "Quelle", "Richtung"];
...
function doGet(e) {
  ...
  if (action) {
    switch (action) {
      ...
      case "ADD_TEST_ENTRY": {
        ...
        const requesterId = p.id || "unknown";
        const saved = appendMeasurementRow(ss, new Date(), speedValue, limitValue, "TEST", "TEST", "unknown");
  ...
  if (speed && !isNaN(speed)) {
    ...
    appendMeasurementRow(ss, time, speedValue, limit, isAlert, source, direction);
...
function handleLegacyEvent(ss, event, params) {
  ...
  if (event.startsWith("radar_alarm_")) {
    ...
    appendMeasurementRow(ss, now, speedValue, limit, "🚨 ALARM (>20%)", source, params.direction || "unknown");
...
function appendMeasurementRow(ss, timestamp, speedValue, limitValue, statusText, sourceText, directionText) {
  if (!ss) ss = SpreadsheetApp.openById(getSpreadsheetId());
...
function getDefaultRadarConfig() {
  return {
    ...
    mount_angle_deg: 12,
    speed_limit_kmh: 50,
    direction_override: -1 // -1 auto, 1 anfahrt, 0 wegfahrt
  };
}
...
function setRadarConfig(ss, params) {
  ...
  const next = Object.assign({}, current, {
    ...
    direction_override: Number(params.direction_override != null ? params.direction_override : current.direction_override)
  });
...
function getLastMeasurementAsJson(ss) {
  ...
  // Annahme: Spalten = [Zeitstempel, Geschwindigkeit, Limit, Überschreitung, Status, Quelle, Richtung]
